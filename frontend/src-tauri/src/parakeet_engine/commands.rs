...
/// setup before `parakeet_init`, and again from `storage_settings::change_models_directory`
...
/// Re-point the Parakeet engine at a new models directory immediately, without an app
...
pub fn reinit_with_dir(root: PathBuf) -> Result<(), String> {
    ...
    *PARAKEET_ENGINE.lock().unwrap() = Some(Arc::new(engine));
    Ok(())
}
